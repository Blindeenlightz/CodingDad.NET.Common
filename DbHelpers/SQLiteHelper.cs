    ...
    public class SQLiteHelper : IDatabaseHelper
    {
        ...
        public virtual async Task CreateTableAsync (string createTableQuery)
        {
            ...
            try
            {
                ...
                LoggerProvider.Log("Table created successfully.", LogLevel.Information);
            }
            catch (Exception ex)
            {
                LoggerProvider.Log($"Failed to create table: {ex.Message}", LogLevel.Error);
        ...
        public async Task<bool> CreateUserAsync (string email, string username, string password)
        {
            ...
            {
                LoggerProvider.Log($"Failed to create user: {ex.Message}", LogLevel.Error);
        ...
        public virtual async Task<bool> DeleteRecordAsync (string tableName, string conditionColumn, object conditionValue)
        {
            ...
            try
            {
                ...
                int result = await ExecuteNonQueryAsync(query, parameters).ConfigureAwait(false);
                LoggerProvider.Log($"Record deleted from {tableName} where {conditionColumn} = {conditionValue}", LogLevel.Information);
                return result > 0;
            }
            catch (SQLiteException ex)
            {
                LoggerProvider.Log($"Failed to delete record from table {tableName}: {ex.Message}", LogLevel.Error);
        ...
        public virtual async Task<bool> InsertIntoTableAsync (string tableName, Dictionary<string, object?> columnData)
        {
            ...
            try
            {
                ...
                LoggerProvider.Log($"Inserted record into {tableName}", LogLevel.Information);
                return result > 0;
            }
            catch (Exception ex)
            {
                LoggerProvider.Log($"Failed to insert record into table {tableName}: {ex.Message}", LogLevel.Error);
        ...
        public virtual async Task<List<string>> RetrieveAllViewModelStatesAsync (string viewModelName)
        {
            ...
            try
            {
                ...
                LoggerProvider.Log($"Retrieved {serializedStates.Count} ViewModel states for {viewModelName}", LogLevel.Information);
                return serializedStates;
            }
            catch (Exception ex)
            {
                LoggerProvider.Log($"Failed to retrieve ViewModel states for {viewModelName}: {ex.Message}", LogLevel.Error);
        ...
        public virtual async Task SaveSingletonViewModelStateAsync (string viewModelName, string serializedState)
        {
            ...
            try
            {
                ...
                LoggerProvider.Log($"Saved singleton ViewModel state for {viewModelName}", LogLevel.Information);
            }
            catch (SQLiteException ex)
            {
                LoggerProvider.Log($"Failed to save ViewModel state for {viewModelName}: {ex.Message}", LogLevel.Error);
        ...
        public virtual async Task SaveViewModelStateAsync (string viewModelName, string instanceId, string serializedState)
        {
            ...
            try
            {
                ...
                await ExecuteNonQueryAsync(query, parameters).ConfigureAwait(false);
                LoggerProvider.Log($"Saved ViewModel state for {viewModelName} with instance ID {instanceId}", LogLevel.Information);
            }
            catch (SQLiteException ex)
            {
                LoggerProvider.Log($"Failed to save ViewModel state for {viewModelName} with instance ID {instanceId}: {ex.Message}", LogLevel.Error);
                throw new SQLiteException("Failed to save ViewModel state.", ex);
            }
        }
        ...
        public virtual async Task<bool> StoreUniqueIdentifierAsync (string userId, string uniqueIdentifier)
        {
            ...
            try
            {
                ...
                int result = await ExecuteNonQueryAsync(query, parameters).ConfigureAwait(false);
                LoggerProvider.Log($"Stored unique identifier for user {userId}", LogLevel.Information);
                return result > 0;
            }
            catch (SQLiteException ex)
            {
                LoggerProvider.Log($"Failed to store unique identifier for user {userId}: {ex.Message}", LogLevel.Error);
        ...
        public async Task<bool> VerifyUserAsync (string email, string password)
        {
            ...
            {
                LoggerProvider.Log($"Failed to verify user {email}: {ex.Message}", LogLevel.Error);
        ...
        private async Task ValidateDatabaseFileExistsAsync ()
        {
            ...
            {
                LoggerProvider.Log($"Database file not found at {dbPath}", LogLevel.Error);
                throw new FileNotFoundException($"Database file not found at {dbPath}");
            }
            LoggerProvider.Log($"Database file found at {dbPath}", LogLevel.Information);
        }
    }
}
